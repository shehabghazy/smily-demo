...
async function getAllUsers() {
    let finalRes
    const res = await fetch('http://localhost:6006/user/allusers' , {
        headers: {
            'userToken': userToken,
          },
    })
    ...
    allUsers = finalRes.allUsers
    console.log(allUsers);
    for (let i = 0; i < allUsers.length; i++) {
        if (allUsers[i]._id == userInfo.userId) {
            allUsers.splice(i,1)
        }
    }
    displayAllUsers()
...
let usersPrepare , usersElem
function displayAllUsers() {
    usersPrepare = ``
    for (let i = 0; i < allUsers.length; i++) {
        usersPrepare += `
        <div class=""><button onclick="getUserId(${i} , this)" class="user-class border-0 btn btn-outline-info w-100 rounded-0">${allUsers[i].userName}</button></div>`
    }
    usersContainer.innerHTML = usersPrepare
    usersElem = Array.from(document.getElementsByClassName('user-class'))
}

function getUserId(getIndex , thisUser) {
    writingPostSection.classList.add('d-none')
    spaceHeaderAssist.classList.remove('d-none')
    console.log(thisUser.innerHTML);
    spaceHeader.innerHTML = `<i class="fa-regular fa-paste mx-2"></i>${thisUser.innerHTML} Posts`
    const userId = allUsers[getIndex]._id
    usersElem.forEach( ele => {
    ...
    async function getUserposts() {
        let finalRes
        const res = await fetch(`http://localhost:6006/posts/${userId}` , {
            headers: {
                'userToken': userToken,
              },
        })
        ...
        // console.log('comments' , allCommentsArr);
...
async function getAllposts() {
    let finalRes
    const res = await fetch('http://localhost:6006/posts' , {
        headers: {
            'userToken': userToken,
          },
    })
